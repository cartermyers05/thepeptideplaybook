...
export function TodayLessonCard({ lesson, currentDay, hasCompletedToday, goalTheme }: TodayLessonCardProps) {
  ...
  const gradientClass = goalTheme?.gradient || "from-rose-300 to-rose-400";
      ...
      <div className="p-6">
        <div className="flex justify-between items-center mb-4">
          <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider flex items-center gap-2">
            {GoalIcon && <GoalIcon className={`w-3.5 h-3.5 ${goalTheme?.iconColor || 'text-gray-400'}`} />}
            Today's Lesson
          </span>
